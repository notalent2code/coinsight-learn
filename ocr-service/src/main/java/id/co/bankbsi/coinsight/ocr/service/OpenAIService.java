...
public class OpenAIService {
  ...
  public TransactionCreationRequest processReceiptText(
      String receiptText, Integer suggestedCategoryId, List<CategoryDto> categories) {
    try {
      ...
      if (responseContent.contains("INVALID_RECEIPT")) {
        throw new OcrProcessingException(
            "The provided image does not appear to be a valid receipt or transaction evidence");
      }
      ...
      try {
        ...
        Map<String, Object> keywordCategory =
            categoryService.determineCategory(receiptText, categories);
        ...
        if (aiCategoryId != null && aiConfidence >= 50 && aiConfidence >= keywordConfidence) {
          ...
          log.info(
              "Using AI categorization (ID: {}, confidence: {}%)", finalCategoryId, aiConfidence);
        } else if (keywordConfidence >= 30) {
          // Use keyword categorization if it has reasonable confidence
          finalCategoryId = keywordCategoryId;
          log.info(
              "Using keyword categorization (ID: {}, confidence: {}%)",
              finalCategoryId, keywordConfidence);
  ...
  private String extractDescriptionFromText(String text) {
    try {
      ...
      for (String line : lines) {
        String lowerLine = line.toLowerCase();
        if (lowerLine.contains("catatan")
            || lowerLine.contains("note")
            || lowerLine.contains("deskripsi")
  ...
  private boolean containsTransactionKeywords(String text) {
    String[] keywords = {
      "nominal", "rekening", "total", "biaya", "admin", "bank", "bsi", "struk", "tanggal"
    };
